...
class Calldata {
    ...
    async initDict (size, wallet) {
        ...
        for (let i = 0; i < this.dict.length; i++) {
            const value = this.dict[i].slice(2);
            this.lookup[value] = i;
            this.lookup[value.slice(-8)] = i;
            this.lookup[value.slice(-40)] = i;
            this.lookup[value.slice(-62)] = i;
        }
    }
    ...
    checkStorageCase (n) {
        if (!this.dict || !this.lookup) {
            throw Error('You didn\'t init dict. Use method .initDict() first');
        }
